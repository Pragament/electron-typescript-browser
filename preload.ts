...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const timelapseIntervalInput = document.getElementById("timelapse-interval") as HTMLInputElement;
  ...
  function addNewTab(url = "https://github.com") {
    ...
    const newTab: Tab = { id: tabId, webview, tabElement: tabButton };
    tabs.push(newTab);
    ...
    webview.addEventListener("did-finish-load", () => {
      if (activeTabId === tabId) applyWebviewContent(webview);
    });

    webview.addEventListener("did-navigate", (e) => {
      if (activeTabId === tabId) addressBar.value = e.url;
    });
